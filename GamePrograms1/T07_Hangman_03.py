...
#   is not displayed in the word that you guessed. Want to fix that.
#   When all letters are guessed, displaying the board again should fix it. 
# Modified by Steve August 31, 2019 (version 2) 
# [-1-] Displayed the game board a final time when all letters have been guessed. 
#       Game looks more complete, all guesses and '_' updated with the final guess.
# [-2-] Added 4 more guesses, (and feet and handd to the hangman). Now it lets you
#       have 10 guesses instead of 6.  If a larger list of words were added, it 
#       would be better to have 10 guesses.
# Modified by Steve September 6, 2019 (version 3)
#       Modified function getRandomWord() to use the random.seed()
#       and random.shuffle() methods, attemping to make it more random.
######################
# 
...
## Modified to use the random.seed() and random.shuffle() methods,
## attemping to make it more random.  Some sources say that seed() is already
## automatically called, using system time (and most likely is). But python docs at ...
## (https://docs.python.org/3.7/library/random.html) ...
## do not actually say that.  
def getRandomWord(wordList):
    random.seed() ## Initialize internal state of the random number generator. No arg, seeds with the current system time.
    random.shuffle(wordList) ## Attempting to make the word selection more random.  
